    ...
    if (seg->list)
    {
        if (seg->list_type == LIST_TYPE_UNDEFINED)
        {
            if (av_match_ext(seg->list, "csv"))
                seg->list_type = LIST_TYPE_CSV;
            else if (av_match_ext(seg->list, "ext"))
                seg->list_type = LIST_TYPE_EXT;
            else if (av_match_ext(seg->list, "m3u8"))
                seg->list_type = LIST_TYPE_M3U8;
            else if (av_match_ext(seg->list, "ffcat,ffconcat"))
                seg->list_type = LIST_TYPE_FFCONCAT;
            else
                seg->list_type = LIST_TYPE_FLAT;
        }
        if (!seg->list_size && seg->list_type != LIST_TYPE_M3U8)
        {
            if ((ret = segment_list_open(s)) < 0)
...
calc_times:
    ...
    if (seg->reference_stream_first_pts != AV_NOPTS_VALUE)
    {
        end_pts += (INT64_MAX - end_pts >= seg->reference_stream_first_pts) ? seg->reference_stream_first_pts : INT64_MAX - end_pts;
    }
...
static int delete_old_segments(SegmentContext *seg)
{
    ...
    while (segment)
    {
        ...
        if (unlink(path) < 0)
        {
            av_log(seg, AV_LOG_ERROR, "failed to delete old segment %s: %s\n", path, strerror(errno));
        }
        av_freep(&path);
        previous_segment = segment;
        segment = previous_segment->next;
...
static const AVOption options[] = {
    ...
    {"delete", "delete segment files that are no longer part of the playlist", 0, AV_OPT_TYPE_CONST, {.i64 = SEGMENT_LIST_FLAG_DELETE}, INT_MIN, INT_MAX, E, .unit = "list_flags"},
